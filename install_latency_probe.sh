#!/usr/bin/env bash
# Установка latency_probe.py — ретроспективный латентностный бюджет большого силоса. Read-only, stdlib.
set -euo pipefail
...
FILE_EOF
cd "$DIR"
python3 -m py_compile analysis/latency_probe.py && echo "[OK] py_compile"
python3 - << 'PY_TEST'
import analysis.latency_probe as lp
G=10**9
assert lp.tip_of({"type":"0x2","maxFeePerGas":hex(100*G),"maxPriorityFeePerGas":hex(3*G)},50*G)==3*G
assert lp.tip_of({"type":"0x0","gasPrice":hex(40*G)},50*G)==0
known={"0xliqwin","0xliqcomp"}; WIN="0xaaa"
blk={"baseFeePerGas":hex(50*G),"transactions":[
  {"hash":WIN,"to":"0xliqwin","type":"0x2","maxFeePerGas":hex(60*G),"maxPriorityFeePerGas":hex(5*G),"transactionIndex":"0x1"},
  {"hash":"0xbbb","to":"0xliqcomp","type":"0x2","maxFeePerGas":hex(60*G),"maxPriorityFeePerGas":hex(9*G),"transactionIndex":"0x2"}]}
cb=lp.classify_block(blk,WIN,known,lambda h:0 if h=="0xbbb" else 1)
assert cb["contested"] and cb["winner_is_top_bidder"] is False and cb["competitors"][0]["reverted"]
assert any("ЛАТЕНТНОСТНЫЙ ЗАЗОР" in x for x in lp.verdict({"blocks":100,"contested_blocks":5,"winner_top_bidder":3,"late_competitor_blocks":40,"median_winner_index":8,"median_block_tx":20,"winner_tip_ratio":1.1}))
assert any("АУКЦИОН" in x for x in lp.verdict({"blocks":100,"contested_blocks":60,"winner_top_bidder":50,"late_competitor_blocks":5,"median_winner_index":0,"median_block_tx":15,"winner_tip_ratio":8.0}))
assert any("СУБ-БЛОЧНАЯ" in x for x in lp.verdict({"blocks":100,"contested_blocks":60,"winner_top_bidder":15,"late_competitor_blocks":5,"median_winner_index":3,"median_block_tx":15,"winner_tip_ratio":1.2}))
print("[OK] тесты: tip / classify_block / три ветки вердикта — прошли")
PY_TEST
echo ">> latency_probe.py установлен. Запуск (публичный RPC, archive НЕ нужен):"
echo "   python3 -m analysis.latency_probe --rpc https://rpc.soniclabs.com --days 7"
