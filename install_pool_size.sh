...
cat > "$DIR/analysis/pool_size.py" << 'FILE_EOF'
#!/usr/bin/env python3
...
FILE_EOF
cd "$DIR"
python3 -m py_compile analysis/pool_size.py && echo "[OK] py_compile pool_size.py"
python3 - << 'PY_TEST'
import analysis.pool_size as ps
from collections import Counter
assert ps.scale_monthly(70,7)==300
INC="0x0094c5"; CH="0x6bcbd4"
evs=[{"silo":"0xB","liquidator":INC,"repay":100.0,"usd":100.0,"symbol":"U","block":i} for i in (1,2,3)]
evs+=[{"silo":"0xB","liquidator":CH,"repay":600.0,"usd":600.0,"symbol":"U","block":4}]
wt=ps.winner_value_table(ps.group_by_silo(evs)["0xb"])
assert wt[0][0]==CH.lower() and abs(wt[0][4]-0.25)<1e-9 and abs(wt[0][5]-600/900)<1e-9
ser=ps.weekly_series([{"silo":"0xB","liquidator":INC,"usd":10.0,"block":b} for b in (10,11,12)]+
                     [{"silo":"0xB","liquidator":INC,"usd":10.0,"block":100}], days=14)
assert len(ser)==2 and ser[0][2]==3 and ser[1][2]==1
print("[OK] value-share (счёт 25%/деньги 67%) + тренд недель — прошли")
PY_TEST
echo ">> pool_size.py v2. Запуск: python3 -m analysis.pool_size --rpc https://rpc.soniclabs.com --chain sonic --days 30"
