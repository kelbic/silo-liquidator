...
cat > "$DIR/analysis/read_fee.py" << 'FILE_EOF'
#!/usr/bin/env python3
...
FILE_EOF
cd "$DIR"
python3 -m py_compile analysis/read_fee.py && echo "[OK] py_compile"
python3 - << 'PY_TEST'
import analysis.read_fee as rf
W = ["0"*64]*17
W[2]="0"*24+"aa"*20; W[3]="0"*24+"bb"*20
W[10]=hex(int(0.85e18))[2:].rjust(64,"0"); W[11]=hex(int(0.90e18))[2:].rjust(64,"0")
W[13]=hex(int(0.05e18))[2:].rjust(64,"0"); W[14]="0"*64
cfg=rf.parse_config("0x"+"".join(W))
assert cfg["liquidationFee"]==int(0.05e18) and cfg["flashloanFee"]==0
assert all(n.startswith("✓") for n in rf.frame_check(cfg,"0x"+"aa"*20))
assert rf.IDX["liquidationFee"]==13
print("[OK] регресс off-by-one (13 vs 14) + рамка — прошли")
PY_TEST
echo ">> read_fee v2. Запуск: python3 -m analysis.read_fee --rpc https://rpc.soniclabs.com"
