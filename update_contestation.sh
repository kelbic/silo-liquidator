...
cat > "$DIR/analysis/contestation.py" << 'FILE_EOF'
#!/usr/bin/env python3
...
if __name__ == "__main__":
    main()
FILE_EOF
touch "$DIR/analysis/__init__.py"
cd "$DIR"
python3 -m py_compile analysis/contestation.py && echo "[OK] py_compile"
python3 -c "import analysis.contestation" && echo "[OK] реальный импорт"
python3 - << 'PY_TEST'
import analysis.contestation as ct
calls = []
class F:
    def call(s, method, params):
        calls.append((method, params)); return []
rpc = F()
ct.fetch_liquidation_logs(rpc, 100, 200)
assert calls[0][1][0]["topics"] == [ct.TOPIC0_LIQUIDATION_CALL]
calls.clear()
BORROW = "0x96558a334f4759f0e7c423d68c84721860bd8fbf94ddc4e55158ecb125ad04b5"
ct.fetch_liquidation_logs(rpc, 100, 200, topic0=BORROW)
assert calls[0][1][0]["topics"] == [BORROW]
print("[OK] fetch_liquidation_logs: старое поведение не изменилось + новый topic0 работает")
PY_TEST
echo ">> contestation.py обновлён (+опциональный topic0 в fetch_liquidation_logs)."
