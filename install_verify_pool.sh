...
cat > "$DIR/analysis/verify_pool.py" << 'FILE_EOF'
#!/usr/bin/env python3
...
FILE_EOF
cd "$DIR"
python3 -m py_compile analysis/verify_pool.py && echo "[OK] py_compile verify_pool.py"
python3 - << 'PY_TEST'
import analysis.verify_pool as vp
assert vp.order_of_magnitude(1500)==3 and vp.order_of_magnitude(0)==-99 and vp.order_of_magnitude(None)==-99
h=vp.histogram_by_oom([{"usd":1.0},{"usd":5.0},{"usd":1_000_000.0},{"usd":None}])
assert h[0]==2 and h[6]==1 and h[-99]==1
assert (1359*10**18)//(10**6)==1359*10**12  # неверный decimals двигает размер на 10^(Δ)
print("[OK] порядок величины + гистограмма-кучкование + подпись артефакта decimals — прошли")
PY_TEST
echo ">> verify_pool.py. Запуск: python3 -m analysis.verify_pool --rpc https://rpc.soniclabs.com --silo 0x322e1d5384aa4ed66aeca770b95686271de61dc3 --days 30"
