...
cat > "$DIR/analysis/borrower_health.py" << 'FILE_EOF'
#!/usr/bin/env python3
...
FILE_EOF
cd "$DIR"
python3 -m py_compile analysis/borrower_health.py && echo "[OK] py_compile"
python3 -c "import analysis.borrower_health" && echo "[OK] реальный импорт"
python3 - << 'PY_TEST'
import analysis.borrower_health as bh
def mk(ltv,lt,debt):
    class F:
        def eth_call(s,to,data):
            if data.startswith(bh.SEL_IS_SOLVENT): return "0x"+"0"*63+"1"
            if data.startswith(bh.SEL_USER_LTV): return "0x"+hex(ltv)[2:].rjust(64,"0")
            if data.startswith(bh.SEL_USER_LT): return "0x"+hex(lt)[2:].rjust(64,"0")
            if data.startswith(bh.SEL_DEBT_BAL): return "0x"+hex(debt)[2:].rjust(64,"0")
    return bh.get_borrower_health(F(),"0xsilo","0xb")
h1=mk(0,0,0); assert h1["debt_raw"]==0 and h1["ltv"]==0
h2=mk(int(0.6e18),int(0.8e18),3000_000000); assert h2["debt_raw"]>0 and h2["ltv"]>0
h3=mk(0,0,5000_000000); assert h3["debt_raw"]>0 and h3["ltv"]==0
print("[OK] 3 сценария (закрыта/здоров/несогласовано) декодируются верно — прошли")
PY_TEST
echo ">> borrower_health.py v3 (футер: НЕТ ПОЗИЦИИ / ДАННЫЕ НЕДОСТОВЕРНЫ вместо ложного 'здоров'). Пример:"
echo "   python3 -m analysis.borrower_health --rpc https://rpc.soniclabs.com --silo 0x322e1d5384aa4ed66aeca770b95686271de61dc3 --borrower <адрес>"
