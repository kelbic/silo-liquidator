...
[ -f "$DIR/analysis/latency_probe.py" ] || { echo "СТОП: нет latency_probe.py"; exit 1; }
grep -q '"idx":' "$DIR/analysis/latency_probe.py" || { echo "СТОП: latency_probe.py без idx-поля"; exit 1; }
cat > "$DIR/analysis/winner_probe.py" << 'FILE_EOF'
#!/usr/bin/env python3
...
FILE_EOF
cd "$DIR"
python3 -m py_compile analysis/winner_probe.py && echo "[OK] py_compile"
python3 - << 'PY_TEST'
import analysis.latency_probe as lp
G=10**9
WIN="0xccd487e01e9df6932f656b53668f58005f604417"
PIPELINE="0xa2712025f69c1f1538f7428269e998f9777d7c96"
EXTERNAL="0xdeadbeef00000000000000000000000000dead"
known={WIN.lower(),PIPELINE,EXTERNAL}
def mk(wt,comps):
    txs=[{"hash":"0xw","to":WIN,"type":"0x0","gasPrice":hex(wt+1),"transactionIndex":"0x0"}]
    st={"0xw":1}
    for i,(to,t,rev,idx) in enumerate(comps):
        h=f"0xc{i}"; txs.append({"hash":h,"to":to,"type":"0x0","gasPrice":hex(t+1),"transactionIndex":hex(idx)}); st[h]=0 if rev else 1
    return lp.classify_block({"baseFeePerGas":"0x1","transactions":txs},"0xw",known,lambda h: st.get(h,1))
cb1=mk(100*G,[(PIPELINE,100*G,True,5)]); cb2=mk(200*G,[(EXTERNAL,200*G,True,3)])
contested=[({"block":1,"usd":1000},cb1),({"block":2,"usd":1000},cb2)]
pipeline_addrs={PIPELINE}; BID_FLOOR=10*G
gt2=eq2=lt2=0
for e,cb in contested:
    rb=[c["tip"] for c in cb["competitors"] if c["reverted"] and c["tip"]>BID_FLOOR and c["to"] not in pipeline_addrs]
    if not rb: continue
    m=max(rb); wt=cb["winner_tip"] or 0
    if wt>m: gt2+=1
    elif wt==m: eq2+=1
    else: lt2+=1
assert (gt2,eq2,lt2)==(0,1,0)
print("[OK] pipeline-фильтр отсеивает ногу, сохраняет внешний сигнал — прошли")
PY_TEST
echo ">> winner_probe.py v6 (+pipeline-исключённый сплит и idx рядом со старыми). Тот же запуск:"
echo "   python3 -m analysis.winner_probe --rpc https://rpc.soniclabs.com --silo 0x322e1d5384aa4ed66aeca770b95686271de61dc3 --winner 0xccd487e01e9df6932f656b53668f58005f604417 --min-usd 100 --days 30"
