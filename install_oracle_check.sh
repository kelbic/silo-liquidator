...
cat > "$DIR/analysis/oracle_check.py" << 'FILE_EOF'
#!/usr/bin/env python3
...
FILE_EOF
cd "$DIR"
python3 -m py_compile analysis/oracle_check.py && echo "[OK] py_compile"
python3 -c "import analysis.oracle_check" && echo "[OK] реальный импорт"
python3 - << 'PY_TEST'
import analysis.oracle_check as oc
assert oc.SEL_CONFIG == "0x79502c55"
assert oc.SEL_GETCONFIG == "0xe48a5f7b"
assert oc.SEL_ORACLE_CONFIG == "0x324b8d6e"
assert oc.SEL_CHAINLINK_CONFIG == "0xc3f909d4"
assert oc.SEL_GETCONFIG != oc.SEL_CHAINLINK_CONFIG
def word_addr(tag): return "0"*24+tag.lower().ljust(40,"0")
def word_uint(n): return hex(n)[2:].rjust(64,"0")
words=[word_uint(650),word_uint(100),word_addr("f55902de"),word_addr("token0000"),
       word_addr("protshare"),word_addr("collshare"),word_addr("debtshare"),
       word_addr("5da3510d"),word_addr("maxltvora"),word_addr("irm000000"),
       word_uint(int(0.85e18)),word_uint(int(0.80e18)),word_uint(int(0.78e18)),
       word_uint(int(0.065e18)),word_uint(0),word_addr("6aafd9dd"),word_uint(0)]
ret="0x"+"".join(words)
class F:
    def eth_call(s,to,data):
        assert data.startswith(oc.SEL_GETCONFIG); return ret
result=oc.get_solvency_oracle(F(),"0xconfig","0xf55902de87bd80c6a35614b48d7f8b612a083c12")
expected="0x"+word_addr("5da3510d")[24:]
assert result==expected
print("[OK] 4 селектора (литеральное сравнение, БЕЗ Crypto) + end-to-end decode — прошли")
PY_TEST
echo ">> oracle_check.py готов (тест без Crypto-зависимости). Запуск:"
echo "   python3 -m analysis.oracle_check --rpc https://rpc.soniclabs.com --silo 0xf55902de87bd80c6a35614b48d7f8b612a083c12"
